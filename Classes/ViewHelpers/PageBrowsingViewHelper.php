...
class PageBrowsingViewHelper extends AbstractTagBasedViewHelper
{
    ...
    public function render()
    {
        ...
        if ($currentPage > 0) {
            ...
            $content .= '<li class="nav-item">' . $this->makecurrentPageSelector_link($label, $currentPage - 1, $freeIndexUid) . '</li>';
        }
        ...
        for ($a = $minPage; $a <= $maxPage; $a++) {
            ...
            if ($a === $currentPage) {
                $content .= '<li class="nav-item"><strong>' . $label . '</strong></li>';
            } else {
                $content .= '<li class="nav-item">' . $label . '</li>';
            }
        }
        // next link
        ...
        if (!$this->tag->hasAttribute('class')) {
            $this->tag->addAttribute('class', 'nav nav-pills nav-fill');
        }
    ...
    protected function makecurrentPageSelector_link($str, $p, $freeIndexUid)
    {
        ...
        return '<a href="#" class="nav-link" onclick="' . htmlspecialchars($onclick) . '">' . htmlspecialchars($str) . '</a>';
    }
}
